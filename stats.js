...
var tags = {}; // add a tags collection to support datadog styles tags
    ...
    backendEvents.once('flush', function clear_metrics(ts, metrics) {
        ...
        for (var counter_key in metrics.counters) {
            if (conf.deleteCounters) {
                if ((counter_key.indexOf("packets_received") != -1) || (counter_key.indexOf("metrics_received") != -1) || (counter_key.indexOf("bad_lines_seen") != -1)) {
    ...
    if (!serverLoaded) {
        ...
        serverLoaded = startServer(config, server, function(msg, rinfo) {
            ...
            for (var midx in metrics) {
                ...
                // wp: specific
                m_str = metrics[midx].toString();
                // check if this pattern |# present in the reported metric string
                if (m_str.indexOf('|#') > 0) {
                    m_tags = m_str.split('|#')[1]; // extract the tag string
                    bits = m_str.split('|#')[0].split(':'); // separate bits
                } else {
                    bits = m_str.split(':');
                }
                
                key = sanitizeKeyName(bits.shift());
                // If tags present add it to tags collection with the key
                if(m_tags){
                   tags[key] = m_tags.split(',');
                }
                ...
                switch (cmd) {
                    ...
                    case "stats":
                        ...
                        backendEvents.emit('status', function(err, name, stat, val) {
                            if (err) {
                                l.log("Failed to read stats for backend " + name + ": " + err);
                            } else {
        ...
        if (keyFlushInterval > 0) {
            ...
            keyFlushInt = setInterval(function() {
                ...
                if (keyFlushLog) {
                    var logFile = fs.createWriteStream(keyFlushLog, {
                        flags: 'a+'
                    });
